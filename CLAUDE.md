...
**Environments**: `src/environments/environment.type.ts` defines the `Environment` interface; `environment.ts` / `environment.prod.ts` implement it. Production builds file-replace `environment.ts` with `environment.prod.ts` (configured in `angular.json`). Content that's real but likely to change independently of code — currently `socialLinks: { name: IconName; url: string }[]` (consumed by `Sidebar`) — lives here rather than hardcoded in a component, and must be duplicated in both `environment.ts` and `environment.prod.ts` (there's no shared base between them).
